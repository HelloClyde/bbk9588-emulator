...
2. 准备自己的 `bbk9588_nand.bin`，或从
   ...
   `.bin` 或 `.zip`。
3. 双击 `start-web.cmd`，Web 服务就绪后浏览器会打开 <http://127.0.0.1:8000/>。
...
Web 默认启用自适应 instruction clock，使 TCU 定时在视频解码等高负载下仍与 guest
执行进度同步；时序对比测试可传入 `--no-qemu-icount` 关闭。
...
- 专用 JZ4740/XBurstR1 CPU 型号，以及 INTC、TCU、CPM、DMAC、AIC/internal codec
  等独立 QEMU 设备模型。
...
每个 Web 实例会持有该路径的跨进程独占锁，同一 NAND 不能被两个模拟器
同时打开。文件管理和 `-Nand` 导入都先构造临时候选镜像，校验 FTL、FAT
和目标内容后再原子替换；校验失败时原镜像保持不变。
